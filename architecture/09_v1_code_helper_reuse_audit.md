...
- `uHOME-empire/src/` as a small provider payload normalization helper
- input adapters behind Google, HubSpot, and generic webhook contracts

Reason:

- the logic is portable and still matches the current `uHOME-empire` role
...
- `uHOME-empire/src/` as contact merge and mirror-sync helper logic
...
- `uHOME-empire/src/` as a narrow validation helper used by contact ingest and
...
- `uHOME-empire/src/` as explicit transform pipelines or provider mapping
  stages
...
- `uHOME-empire/scripts/` for local development helpers only, if needed
...
- `uHOME-empire` docs and future runtime design as a data-shape reference only

Reason:

- the table partitioning is conceptually useful, but v2 ownership is now
  distributed across the macOS app, `uHOME-empire`, and shared family
  contracts
...
1. `uHOME-empire` normalization helper
2. `uHOME-empire` dedupe and validation helper set
3. `uHOME-empire` explicit transform or enrichment pipeline
