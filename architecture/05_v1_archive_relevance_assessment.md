...
- `uHOME-empire` cron-type webhooks and sync jobs must consume core workflow contracts
...
- `uHOME-empire` should use Wizard networking contracts for remote APIs rather than inventing parallel transport policy
...
- `uHOME-empire` should teach configurable webhook and API server patterns
...
- repo-specific ownership into `uHOME-empire`, `uHOME-server`, `uDOS-wizard`, or the macOS app
